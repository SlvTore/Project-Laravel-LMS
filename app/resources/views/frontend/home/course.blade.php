...
<section class="course-area pb-120px">
    <div class="container">
        ...
        <ul class="nav nav-tabs generic-tab justify-content-center pb-4" id="myTab" role="tablist">
            ...
            <li class="nav-item">
                <a class="nav-link" id="{{ $category->category_slug }}-tab" data-toggle="tab" href="#{{ $category->category_slug }}" role="tab" aria-controls="{{ $category->category_slug }}" aria-selected="false">{{ $category->category_name }}</a>
            </li>
            @endforeach
        </ul>
    </div><!-- end container -->
            ...
            <div class="tab-content" id="myTabContent">
                ...
                <div class="tab-pane fade show active" id="all" role="tabpanel" aria-labelledby="all-tab">
                    <div class="row">
                        ...
                        <div class="col-lg-4 responsive-column-half">
                            <div class="card card-item card-preview">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="card-text"><a href="javascript:;">{{ $course->user->name }}</a></p>
                        ...
                        @empty
                        <div class="text-danger ps-3">No Course Found</div>
                        @endforelse
                    </div><!-- end row -->
                </div><!-- end tab-pane -->
                ...
                @foreach ($categories as $category)
                <div class="tab-pane fade" id="{{ $category->category_slug }}" role="tabpanel" aria-labelledby="{{ $category->category_slug }}-tab">
                    <div class="row">
                        @php
                            $cat_courses = App\Models\Course::where('category_id', $category->id)->where('status', 1)->orderBy('id', 'DESC')->get();
                        @endphp
                        
                        @forelse ($cat_courses as $course)
                        <div class="col-lg-4 responsive-column-half">
                            <div class="card card-item card-preview">
                                <div class="card-image">
                                    <a href="{{ url('course/details/'.$course->id.'/'.$course->course_name_slug) }}" class="d-block">
                        ...
                        @empty
                        <div class="text-danger ps-3">No Course Found in this Category</div>
                        @endforelse
                    </div><!-- end row -->
                </div><!-- end tab-pane -->
                @endforeach
